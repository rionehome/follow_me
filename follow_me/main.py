...
class FollowMe(Node):
    ...
    def __init__(self):
        ...
        ## User configurable ##
        self.view_laser:  bool = True  # whether to show graphical lidar data
    ...
    def ydlidar_callback(self, msg) -> None:
        ...
        self.get_logger().info(f"Delta [m] : {np.sqrt(np.power(delta_x, 2) + np.power(delta_y, 2))}")
    ...
    def view_ydlidar(self, points: list[Point]) -> None:
        ...
        cv2.circle(img, center=(int(self.player_point.x) + 500, int(self.player_point.y) + 250), radius=5, color=(0, 0, 255), thickness=1)
